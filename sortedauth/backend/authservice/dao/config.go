...
type OAuthProviderConfig struct {
	...
	RedirectURL  string   `koanf:"redirect_url" json:"redirect_url"`
	IssuerURL    string   `koanf:"issuer_url" json:"issuer_url"`
	Scopes       []string `koanf:"scopes" json:"scopes"`
	AuthURL      string   `koanf:"auth_url" json:"auth_url"`
	TokenURL     string   `koanf:"token_url" json:"token_url"`
}

// OAuthConfig holds configuration for all OAuth providers
type OAuthConfig struct {
	Providers map[string]OAuthProviderConfig `koanf:"providers" json:"providers"`
}

// Config represents the complete application configuration
type Config struct {
	Database DatabaseConfig `koanf:"database" json:"database"`
	OAuth    OAuthConfig    `koanf:"oauth" json:"oauth"`
}
...
func loadFromEnv(k *koanf.Koanf) {
	...
	// OAuth Configuration
	// Generic OAuth Provider Loading (e.g. OAUTH_PROVIDER_GITHUB_CLIENT_ID)
	// This allows adding more providers via env vars like OAUTH_PROVIDER_<NAME>_<FIELD>
	for _, env := range os.Environ() {
		pair := strings.SplitN(env, "=", 2)
		key := pair[0]
		value := pair[1]

		if strings.HasPrefix(key, "OAUTH_PROVIDER_") {
			parts := strings.Split(strings.ToLower(key), "_")
			if len(parts) >= 4 {
				// Format: OAUTH_PROVIDER_<NAME>_<FIELD>
				// parts[0] = oauth, parts[1] = provider
				providerName := parts[2]
				fieldName := strings.Join(parts[3:], "_") // join rest as field name

				if fieldName == "scopes" {
					k.Set(fmt.Sprintf("oauth.providers.%s.%s", providerName, fieldName), strings.Fields(value))
				} else {
					k.Set(fmt.Sprintf("oauth.providers.%s.%s", providerName, fieldName), value)
				}
			}
		}
	}
