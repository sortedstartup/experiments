...
func main() {
	...
	tenantServiceDao, err := authDaoFactory.CreateTenantDAO()
	if err != nil {
		log.Fatalf("Failed to create tenant service DAO: %v", err)
	}
	...
	tenantService := authService.NewTenantService(tenantServiceDao, userServiceDao)

	// Create TokenService
	tokenService := authService.NewTokenService(jwtSecret, issuer, 24*time.Hour)

	// Initialize providers
	providers, err := authService.InitProviders(authConfig)
	if err != nil {
		log.Fatalf("Failed to initialize providers: %v", err)
	}

	// Creating APIs
	userServiceAPI := authApi.NewUserServiceAPI(userService, tenantService)
	tenantServiceAPI := authApi.NewTenantServiceAPI(tenantService)

	authSvc := authService.NewAuthService(userService, tokenService, providers)

	// Create HTTP mux and register HTTP routes
	mux := http.NewServeMux()
	authServiceApi := authApi.NewAuthAPI(mux, authSvc)
	authServiceApi.Init()
	...
	proto.RegisterTenantServiceServer(grpcServer, tenantServiceAPI)
